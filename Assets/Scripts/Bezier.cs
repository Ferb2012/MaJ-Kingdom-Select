...
public class Bezier : MonoBehaviour
{
    ...
    private void DrawQuadraticCurve()
    {
        for (int i = 0; i < numPoints; i++)
        {
            ...
            positions[i] = CalulateQuadraticBezierPoint(t, points[0].position, points[1].position, points[2].position);
        }

        line.SetPositions(positions);
    }

    private Vector3 CalulateQuadraticBezierPoint(float t, Vector3 p0, Vector3 p1, Vector3 p2)
    {
        float u = 1 - t;
        float tt = t * t;
        float uu = u * u;
